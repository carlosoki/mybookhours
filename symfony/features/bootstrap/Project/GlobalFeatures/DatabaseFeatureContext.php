...
use Project\Bundle\Api\Entity\OpportunityGroup;
use Project\Bundle\Api\Entity\Company;
use Project\Bundle\Api\Entity\CronTask;
use Project\Bundle\Api\Entity\Opportunity;
use Project\Bundle\Api\Entity\OpportunitySelectedProfile;
use Project\Bundle\Api\Entity\Profile;
use Project\Bundle\Api\Entity\ProfileExtension;
use Project\Bundle\Api\Entity\QuestionAnswer;
use Project\Bundle\Api\Entity\Offer;
...
class DatabaseFeatureContext implements Context, SnippetAcceptingContext, KernelAwareContext
{
    ...
    public function gatherContexts(BeforeScenarioScope $scope)
    {
        ...
        $this->locationContext = $environment->getContext('Project\GlobalFeatures\LocationContext');
    }
    ...
    protected function retrieveRelation($relationName, $id)
    {
        ...
        switch($relationName){
            ...
            case 'admittedToPracticeLocations':
                $className = "ProjectLocationBundle:Location";
                break;
            ...
            case 'invitedOffices':
                $className = "ProjectApiBundle:Office";
                break;
            ...
            case 'approver':
            case 'createdBy':
                $className = "ProjectApiBundle:User";
                break;

            case 'rateCurrency':
                $className = "ProjectApiBundle:Currency";
                break;

            case 'offer':
                $className = "ProjectApiBundle:Offer";
                break;

            case 'secondment':
                $className = "ProjectApiBundle:OpportunitySecondment";
                break;

            case 'logo':
            case 'tcFile':
                $className = "ProjectApiBundle:FileUpload";
                break;
...
Chupa chups tiramisu lemon drops soufflé cheesecake apple pie gummi bears. Jelly beans biscuit biscuit cake tart tootsie roll sugar plum soufflé. Chupa chups pastry cookie. Bonbon bonbon danish toffee macaroon. Gummies dessert pastry marzipan cake croissant tart soufflé wafer. Carrot cake pastry jelly sesame snaps pie candy canes. Oat cake jelly chocolate cake candy sugar plum. Gingerbread cheesecake chocolate bar. Sugar plum pastry pie jelly toffee gummies croissant marzipan. Powder cotton candy carrot cake croissant.',
    ...
    public function opportunityHasCompanies($arg1, $arg2)
    {
        ...
        $repo = $em->getRepository("ProjectApiBundle:Opportunity");
    ...
    public function fromBundleCheckIfIdWasFound($arg1, $arg2, $id, $count)
    {
        $em = $this->getEntityManager();
        $em->clear();
        $entityNamespace = 'Project\Bundle\\'.$arg1.'\Entity\\'.$arg2;
    ...
    public function checkOpportunityAndFieldWasChanged($id, $field, $search, $count)
    {
        ...
        $em = $this->getEntityManager();
        $em->clear();
        $entityNamespace = 'Project\Bundle\Api\Entity\Opportunity';
    ...
    public function userFromOfficeShouldBeAssignedToOpportunity($arg1)
    {
        ...
        $userRepo = $em->getRepository("ProjectApiBundle:User");
        $officeUsers = $userRepo->userSupplierToOfferPerOffices($offices, $opportunity->getProfession());

        $assignEntity = $em->getRepository("ProjectApiBundle:OpportunityAssign");
